...
export interface ExerciseInstance {
  ...
  weightLabel?: string;  // prescription en texte libre : charge ("80", "70%", "RPE 8") ou allure CAP ("4:30 à 4:35") — prime sur weight à l'affichage
...
export interface SessionInstance {
  ...
  clientComment?: string; // commentaire global du sportif sur la séance
...
export interface Challenge {
  ...
  title: string;
  description: string;
  // `exId` : requis pour "exercise_weight" (→ LibraryExercise.id ciblé).
  // `requiredIds` : requis pour "badge_set" (→ liste de Challenge.id prérequis).
  condition: { type: ChallengeConditionType; value: number; exId?: string; requiredIds?: string[] };
